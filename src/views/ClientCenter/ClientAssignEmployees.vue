...
export default {
  ...
  methods: {
    ...
    onSelectChange(val, flag){
      // flag为true时往this.memberSelected里面添加值，为false时根据empId删除值
      if(flag){
        this.memberSelected.push(val)
      }else{
        this.memberSelected.splice(this.memberSelected.findIndex(item => item.empId === val.empId), 1)
      }
    }
  }
};
